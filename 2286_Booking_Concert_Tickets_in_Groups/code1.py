...
class SegmentTree:
    ...
    def queryMax(self, leftIndex, rightIndex):
        ...
        def _query(root, leftIndex, rightIndex):
            ...
            if midIndex >= rightIndex:
                ...
                return max(_query(root.left, leftIndex, midIndex), _query(root.right, midIndex + 1, rightIndex))
...
class BookMyShow:
    ...
    def gather(self, k: int, maxRow: int) -> List[int]:  
        left = self.scatterRow
        right = maxRow + 1
        while left < right:
            mid = left + (right - left) // 2
            if self.ST.queryMax(left, mid) < k:
                left = mid + 1
            else:
                right = mid
        ...
        start = self.m - self.ST.getValue(left)
        self.ST.update(left, k)
