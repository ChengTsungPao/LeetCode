class Solution:
    def canMouseWin(self, grid: List[str], catJump: int, mouseJump: int) -> bool:
        ...
        def countChildren(mousePos, catPos, turn, memoMouse = {}, memoCat = {}):
            if turn == MOUSETURN:
                if mousePos not in memoMouse:   
                    count = 0
                    i, j = mousePos
                    for c1, c2 in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                        for k in range(mouseJump + 1):
                            ...
                            if not (0 <= x < m and 0 <= y < n) or grid[x][y] == "#":
                                break
                            count += 1
                    memoMouse[mousePos] = count
                return memoMouse[mousePos]
            else:
                if catPos not in memoCat:            
                    count = 0
                    i, j = catPos
                    for c1, c2 in [(1, 0), (0, 1), (-1, 0), (0, -1)]:
                        for k in range(catJump + 1):
                            ...
                            if not (0 <= x < m and 0 <= y < n) or grid[x][y] == "#":
                                break
                            count += 1
                    memoCat[catPos] = count
                return memoCat[catPos]
        ...
        allPosition = [(i, j) for i in range(m) for j in range(n)]
        
        for i, j in allPosition:
            if grid[i][j] == "M":
                startMousePos = (i, j)
            elif grid[i][j] == "C":
                startCatPos = (i, j)
        ...
        # Initial degree
        degree = {}
        for mousePos in allPosition:
            for catPos in allPosition:
                degree[mousePos, catPos, MOUSETURN] = countChildren(mousePos, catPos, MOUSETURN)
                degree[mousePos, catPos, CATTURN] = countChildren(mousePos, catPos, CATTURN)                    
        ...
        while que:
            ...
            while que:
                ...
                for preMousePos, preCatPos, preTurn in allParents(mousePos, catPos, turn):
                    ...
                    elif preTurn == CATTURN and win == MOUSEWIN:
                        degree[preMousePos, preCatPos, preTurn] -= 1
                    ...
                    if preTurn == CATTURN and win == CATWIN:
                        ...
                        newQue.appendleft((preMousePos, preCatPos, preTurn))
                    elif preTurn == MOUSETURN and win == CATWIN:
                        degree[preMousePos, preCatPos, preTurn] -= 1
                        if degree[preMousePos, preCatPos, preTurn] == 0:
                            result[preMousePos, preCatPos, preTurn] = CATWIN
                            newQue.appendleft((preMousePos, preCatPos, preTurn))
